...
export const getAllTrxLeaveQuota = async (
  ...
  try {
    ...
    if (isAdmin) {
      ...
      rawLeaveQuota = await TrxLeaveQuota.findMany({
        where: whereClause,
      ...
      totalItems = await TrxLeaveQuota.count({ where: whereClause });
    } else {
      // 1. Ambil data quota aktif milik user
      const leaveQuotas = await TrxLeaveQuota.findMany({
        where: {
          is_deleted: 0,
          id_user: userNrp,
      ...
      // 2. Ambil semua leave type yang tidak membutuhkan kuota
      const nonQuotaLeaveTypes = await LeaveTypes.findMany({
        where: {
          is_quota_needed: 1,
        },
        select: {
      ...
      const existingLeaveTypeIds = leaveQuotas.map(
        (lq) => lq.leaves_type_id
      );

      // 3. Gabungkan tipe cuti non-quota yang belum ada di TrxLeaveQuota user ini
      const additionalNonQuota = nonQuotaLeaveTypes
        .filter((lt) => !existingLeaveTypeIds.includes(lt.id))
        .map((lt) => ({
          id: null,
          leaves_type_id: lt.id,
          leaves_quota: null,
          valid_from: null,
          valid_to: null,
          id_user: userNrp,
      ...
      rawLeaveQuota = [...leaveQuotas, ...additionalNonQuota];
      totalItems = rawLeaveQuota.length;
    }
