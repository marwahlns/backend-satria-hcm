...
export const generatePdfMutation = (res: any, data: any[]) => {
  ...
  res.setHeader("Content-Type", "application/pdf");
  res.setHeader("Content-Disposition", "inline; filename=surat-mutasi.pdf");
  doc.pipe(res);
  ...
  data.forEach((item, index) => {
    ...
    doc.moveDown(5); // Memberikan ruang setelah logo
    ...
    // Update posisi Y setelah informasi pembuat surat
    doc.y = currentY + 30;
    ...
    // Bagian informasi karyawan yang dimutasi
    currentY = doc.y;
    doc.text("Nama", labelX, currentY);
    doc.text(":", colonX, currentY);
    doc.text(item.name, valueX, currentY);
    ...
    currentY += 15;
    doc.text("Divisi Asal", labelX, currentY);
    doc.text(":", colonX, currentY);
    doc.text(item.divisionFrom, valueX, currentY);

    currentY += 15;
    doc.text("Departemen Asal", labelX, currentY);
    doc.text(":", colonX, currentY);
    doc.text(item.deptFrom, valueX, currentY);
    ...
    // Bagian informasi divisi/departemen baru
    currentY = doc.y;
    doc.text("Divisi Baru", labelX, currentY);
    doc.text(":", colonX, currentY);
